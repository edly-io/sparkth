...
**Files:** `app/lib/hooks.py`, `app/lib/mcp/hooks.py`, `app/lib/routes.py`, `app/lib/config/hooks.py`, `app/lib/analytics/hooks.py`, `app/core_plugins/*/plugin.py`
...
SparkthPlugin.__init__
  ...
  └── ANALYTICS_SCHEMAS.add_item(self, MyEvent)                      # app/lib/analytics/hooks.py
```
...
(`app/lib/analytics/hooks.py`): self-describing `AnalyticsEventSchema` subclasses that
declare their own `event_type`/`version`. `initialize_event_registry`
(`app/lib/analytics`) drains the hook into the `EventRegistry` singleton during
