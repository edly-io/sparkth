...
class UserPlugin(SQLModel, table=True):
    ...
    class Config:
        ...
        # Ensure we can have only one preference per user per plugin
        schema_extra = {
            "indexes": [
                # Unique constraint: one preference per user per plugin
                {"name": "idx_user_plugin_unique", "columns": ["user_id", "plugin_name"], "unique": True}
            ]
        }
