...
class LMSCredentialsBuilder:
    """Builds LMS credential system messages for the LLM."""

    @staticmethod
    def lms_tool_prefixes() -> tuple[str, ...]:
    ...
    @staticmethod
    def has_lms_tools(tools: list[Any]) -> bool:
        """Return True if any tool name starts with an LMS-specific prefix."""
        prefixes = LMSCredentialsBuilder.lms_tool_prefixes()
    ...
    async def build_message(
        ...
        if not tools or not LMSCredentialsBuilder.has_lms_tools(tools):
            return None
...
_lms_tool_prefixes = LMSCredentialsBuilder.lms_tool_prefixes
_has_lms_tools = LMSCredentialsBuilder.has_lms_tools
build_lms_credentials_message = LMSCredentialsBuilder.build_message
