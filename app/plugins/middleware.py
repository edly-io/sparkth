...
class PluginAccessMiddleware(BaseHTTPMiddleware):
    ...
    async def _check_plugin_access(self, user_id: int, plugin_name: str) -> bool:
        ...
        try:
            ...
            if result is None:
                logger.debug(
                    f"No UserPlugin record for user {user_id} and plugin '{plugin_name}'. Allowing access by default."
                )
                return True
            ...
            logger.error(f"Error checking plugin access for user {user_id} and plugin '{plugin_name}': {e}")
...
def check_user_plugin_access(user_id: int, plugin_name: str, session: Session) -> bool:
    ...
    try:
        statement = select(UserPlugin).where(
            UserPlugin.user_id == user_id, UserPlugin.plugin_name == plugin_name, UserPlugin.deleted_at.is_(None)
        )
        result = session.exec(statement).first()
        ...
        logger.error(f"Error in check_user_plugin_access for user {user_id} and plugin '{plugin_name}': {e}")
        return False
...
async def get_user_enabled_plugins(user_id: int, session: Session) -> list[str]:
    ...
    try:
        ...
        statement = select(UserPlugin).where(UserPlugin.user_id == user_id, UserPlugin.deleted_at.is_(None))
...
async def get_user_disabled_plugins(user_id: int, session: Session) -> list[str]:
    ...
    try:
        statement = select(UserPlugin).where(
            UserPlugin.user_id == user_id, not UserPlugin.enabled, UserPlugin.deleted_at.is_(None)
        )
        results = session.exec(statement).all()
