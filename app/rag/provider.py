"""Process-level embedding provider singleton."""

from app.core.config import get_settings
from app.core.logger import get_logger
from app.memory_profiler import log_memory_snapshot
from app.rag.constants import DEFAULT_EMBEDDING_DIMENSIONS
from app.rag.embeddings import BaseEmbeddingProvider, get_embedding_provider

logger = get_logger(__name__)


class EmbeddingProviderRegistry:
    """Manages the lifecycle of the process-level embedding provider singleton."""

    def __init__(self) -> None:
        self._provider: BaseEmbeddingProvider | None = None

    def init(self) -> BaseEmbeddingProvider:
        """Create provider and warm model.

        Must be called once during application startup before server begins
        serving requests. Calling it a second time is a no-op (returns existing
        instance).
        """
        if self._provider is not None:
            return self._provider

        # Force single-threaded PyTorch execution before model loading.
        # Default PyTorch uses one OMP thread per CPU core. Each thread creates
        # its own ptmalloc arena. Freed tensor memory fragments those arenas in a
        # way that malloc_trim(0) cannot reclaim (trapped between small allocs).
        # With 1 thread, all matrix ops run in the calling thread — one arena —
        # and malloc_trim fully reclaims freed tensor memory after each inference.
        try:
            import torch  # type: ignore[import-not-found, unused-ignore]
        except ImportError:
            logger.debug("torch not available; skipping num_threads configuration.")
        else:
            if torch.get_num_threads() != 1:
                torch.set_num_threads(1)
            if torch.get_num_interop_threads() != 1:
                # We need to check whether we have already started parallel work,
                # otherwise this fails with the following error: RuntimeError: Error:
                # cannot set number of interop threads after parallel work has started
                # or set_num_interop_threads called
                torch.set_num_interop_threads(1)
            logger.info("PyTorch configured for single-threaded CPU execution (num_threads=1).")

        settings = get_settings()
        provider_name = settings.RAG_EMBEDDING_PROVIDER
        model_name = settings.RAG_EMBEDDING_MODEL

        logger.info(
            "Initializing embedding provider: provider=%s model=%s",
            provider_name,
            model_name,
        )

        log_memory_snapshot(label="before_model_load")

        self._provider = self._build_provider(provider_name, model_name)
        model = self._provider.get_embeddings_model()

        # Run a real warm-up inference to pre-allocate tokenizer vocabulary tables,
        # BLAS workspace buffers, and Python-level tokenizer caches at startup.
        # Without this, these allocations happen during the first production
        # ingestion cycle and appear as unexpected per-cycle memory growth.
        log_memory_snapshot(label="before_warmup_inference")
        model.embed_documents(["warmup"])
        log_memory_snapshot(label="after_model_load")

        logger.info("Embedding provider ready.")
        return self._provider

    def get(self) -> BaseEmbeddingProvider:
        """Return the warmed provider.

        Raises RuntimeError if init() has not been called.
        """
        if self._provider is None:
            raise RuntimeError(
                "Embedding provider not initialized. embedding_provider.init() must be called during application startup."
            )
        return self._provider

    def reset(self) -> None:
        """Reset provider to None. For use in tests only."""
        self._provider = None

    @staticmethod
    def _build_provider(provider_name: str, model_name: str) -> BaseEmbeddingProvider:
        """Construct provider instance. Separated so tests can patch it."""
        return get_embedding_provider(provider_name, model=model_name, dims=DEFAULT_EMBEDDING_DIMENSIONS)


# Process-level singleton
embedding_provider = EmbeddingProviderRegistry()
