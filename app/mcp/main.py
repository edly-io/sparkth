            ...
            try:
                ...
                for tool_def in mcp_tools:
                    try:
                        ...
                        success = _validate_and_register_tool(
                            tool_def, 
                            plugin_name, 
                            registered_tools
                        )
                ...
                if plugin_tool_count > 0:
                    ...
                    logger.warning(
                        f"Plugin '{plugin_name}' failed to register {plugin_failed_count} tool(s)"
                    )
...
def _validate_and_register_tool(
    ...
    if not tool_name:
        logger.warning(
            f"Tool from plugin '{plugin_name}' is missing a name. Skipping."
        )
        return False
    ...
    if not tool_handler:
        logger.warning(
            f"Tool '{tool_name}' from plugin '{plugin_name}' is missing a handler. Skipping."
        )
        return False
    ...
    if not callable(tool_handler):
        logger.warning(
            f"Tool '{tool_name}' from plugin '{plugin_name}' has a non-callable handler. Skipping."
        )
        return False
    ...
    try:
        ...
        logger.info(
            f"  ✓ Registered tool '{tool_name}'{category_str}{version_str} "
        ...
        logger.error(
            f"Failed to register tool '{tool_name}' from plugin '{plugin_name}': {e}"
        )
        return False
