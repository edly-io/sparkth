...
def list_user_plugins(
    ...
    plugins_statement = select(Plugin).where(Plugin.deleted_at == None)
    db_plugins = session.exec(plugins_statement).all()
    plugin_map = {p.name: p for p in db_plugins}
    
    # Get user's plugin settings with join to Plugin table
    statement = select(UserPlugin, Plugin).join(Plugin).where(
    ...
    for plugin_name in available_plugins:
        ...
        if user_plugin:
            ...
            result.append(UserPluginResponse(
