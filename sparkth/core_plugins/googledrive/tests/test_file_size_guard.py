...
from sparkth.core_plugins.googledrive.models import DriveFile
from sparkth.core_plugins.googledrive.utils import _process_single_file
...
async def test_file_exceeding_size_limit_is_marked_failed(session: AsyncSession) -> None:
    ...
    with (
        patch("sparkth.core_plugins.googledrive.utils._download_file", new=AsyncMock(return_value=big_bytes)),
        patch("sparkth.core_plugins.googledrive.utils.get_googledrive_settings") as mock_settings,
        patch("sparkth.core_plugins.googledrive.utils.ingest_document") as mock_ingest,
    ):
...
async def test_pre_download_size_guard_skips_download(session: AsyncSession) -> None:
    ...
    with (
        patch("sparkth.core_plugins.googledrive.utils._download_file") as mock_download,
        patch("sparkth.core_plugins.googledrive.utils.get_googledrive_settings") as mock_settings,
    ):
        mock_settings.return_value.INGESTION_MAX_FILE_SIZE_MB = 50
        await _process_single_file(drive_file, user_id=1, access_token="tok", session=session)
...
async def test_file_within_size_limit_proceeds_to_extraction(session: AsyncSession) -> None:
    ...
    with (
        ...
        patch(
            "sparkth.core_plugins.googledrive.utils._find_ready_duplicate_document_id", new=AsyncMock(return_value=None)
        ),
    ):
        mock_settings.return_value.INGESTION_MAX_FILE_SIZE_MB = 50
