...
from sparkth.core_plugins.googledrive.exceptions import GoogleDriveAPIError
from sparkth.core_plugins.googledrive.models import DriveFile, DriveFolder
from sparkth.core_plugins.googledrive.utils import (
    _download_file,
...
class TestDownloadFile:
    async def test_downloads_with_correct_params(self) -> None:
        ...
        with patch("sparkth.core_plugins.googledrive.utils.GoogleDriveClient") as mock_cls:
...
class TestProcessSingleFile:
    async def test_registers_document_when_none(self) -> None:
        ...
        with (
            patch(
                "sparkth.core_plugins.googledrive.utils.create_document",
                new=AsyncMock(return_value=mock_doc),
            ),
            patch("sparkth.core_plugins.googledrive.utils.update_document_status", new=AsyncMock()),
            patch("sparkth.core_plugins.googledrive.utils._download_file", new=AsyncMock(return_value=b"x")),
            patch(
                "sparkth.core_plugins.googledrive.utils._find_ready_duplicate_document_id",
                new=AsyncMock(return_value=None),
            ),
            patch(
                "sparkth.core_plugins.googledrive.utils.ingest_document",
    ...
    async def test_unsupported_file_marks_document_ready(self) -> None:
        ...
        with (
            patch(
                "sparkth.core_plugins.googledrive.utils.update_document_status",
                new=AsyncMock(),
            ) as mock_update,
            patch("sparkth.core_plugins.googledrive.utils._download_file", new=AsyncMock(return_value=b"x")),
            patch(
    ...
    async def test_none_id_returns_early(self) -> None:
        ...
        drive_file.id = None
        session = _make_async_session()
        with patch("sparkth.core_plugins.googledrive.utils.update_document_status", new=AsyncMock()) as mock_update:
            await _process_single_file(drive_file, 1, "tok", session)
    ...
    async def test_duplicate_file_calls_copy_chunk_links(self) -> None:
        ...
        with (
            ...
            patch(
                "sparkth.core_plugins.googledrive.utils._download_file",
            ...
            patch("sparkth.core_plugins.googledrive.utils.copy_document_chunk_links", new=AsyncMock()) as mock_copy,
        ):
            await _process_single_file(drive_file, 1, "tok", session)
    ...
    async def test_size_guard_drive_reported_marks_document_failed(self) -> None:
        ...
        with (
            ...
            patch("sparkth.core_plugins.googledrive.utils._download_file", new=AsyncMock()) as mock_dl,
        ):
            await _process_single_file(drive_file, 1, "tok", session)
...
class TestProcessFolderRag:
    ...
    @patch("sparkth.core_plugins.googledrive.utils._process_single_file")
