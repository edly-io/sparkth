...
from sparkth.core_plugins.googledrive.models import DriveOAuthToken
from sparkth.core_plugins.googledrive.oauth import (
    decode_state,
    decrypt_token,
    delete_token,
    encrypt_token,
...
class TestSaveTokens:
    @pytest.mark.asyncio
    async def test_save_new_token(self, session: AsyncSession, test_user: User) -> None:
    ...
    @pytest.mark.asyncio
    async def test_save_updates_existing_token(self, session: AsyncSession, test_user: User) -> None:
        """Saving tokens when a record exists should update it."""
        user_id = cast(int, test_user.id)

        await save_tokens(session, user_id, "old_access", "old_refresh", 3600, "scope1")
        record = await save_tokens(session, user_id, "new_access", "new_refresh", 7200, "scope2")
...
class TestGetTokenRecord:
    ...
    async def test_returns_token_when_exists(
        self, session: AsyncSession, test_user: User, test_oauth_token: DriveOAuthToken
    ) -> None:
        """Should return the token record for a connected user."""
        record = await get_token_record(session, cast(int, test_user.id))

        assert record is not None
        assert record.user_id == test_user.id

    @pytest.mark.asyncio
    async def test_returns_none_when_no_token(self, session: AsyncSession, test_user: User) -> None:
...
class TestDeleteToken:
    ...
    async def test_soft_deletes_token(
        ...
        # Capture the id before delete_token commits (which expires the ORM object).
        user_id = cast(int, test_user.id)
        result = await delete_token(session, user_id)
    ...
    @pytest.mark.asyncio
    async def test_returns_false_when_no_token(self, session: AsyncSession, test_user: User) -> None:
        """Delete should return False when no token exists."""
        result = await delete_token(session, cast(int, test_user.id))
...
class TestGetValidAccessToken:
    ...
    async def test_returns_existing_token_if_not_expired(
        ...
        token = await get_valid_access_token(session, cast(int, test_user.id), "client_id", "client_secret")
    ...
    async def test_refreshes_expired_token(
        ...
        with patch(
            "sparkth.core_plugins.googledrive.oauth.refresh_access_token",
    ...
    @pytest.mark.asyncio
    async def test_raises_when_not_connected(self, session: AsyncSession, test_user: User) -> None:
        """Should raise ValueError when user has no token."""
        with pytest.raises(ValueError, match="not connected"):
            await get_valid_access_token(session, cast(int, test_user.id), "client_id", "client_secret")
...
class TestExchangeCodeForTokens:
    ...
    async def test_exchange_success(self) -> None:
        ...
        from sparkth.core_plugins.googledrive.oauth import exchange_code_for_tokens
