...
class TestConnectionStatus:
    ...
    async def test_connected(
        ...
        with patch(
            "sparkth.core_plugins.googledrive.routes.oauth.get_user_info",
...
class TestDisconnect:
    ...
    async def test_disconnect_success(
        ...
        with patch(
            "sparkth.core_plugins.googledrive.routes.oauth.revoke_token",
            new_callable=AsyncMock,
            return_value=True,
        ):
            response = await drive_client.delete("/api/v1/google-drive/oauth/disconnect")
...
class TestSyncFolder:
    ...
    async def test_sync_new_folder(
        ...
        with (
            patch("sparkth.core_plugins.googledrive.routes.folders.GoogleDriveClient") as mock_client_cls,
            patch("sparkth.core_plugins.googledrive.routes.folders.process_folder_rag", new_callable=AsyncMock),
        ):
...
class TestDownloadFile:
    ...
    async def test_download_success(
        ...
        with patch("sparkth.core_plugins.googledrive.routes.files.GoogleDriveClient") as mock_client_cls:
            mock_client = AsyncMock()
    ...
    async def test_download_google_doc_exports_pdf(
        ...
        from sparkth.core_plugins.googledrive.client import GoogleDriveClient as RealClient
