...
from sparkth.core_plugins.googledrive.config import GoogleDriveSettings
from sparkth.core_plugins.googledrive.models import DriveFile, DriveFolder, DriveOAuthToken
...
def _default_ingestion_settings() -> Generator[None, None, None]:
    ...
    with patch("sparkth.core_plugins.googledrive.utils.get_googledrive_settings", return_value=mock_settings):
        yield
...
async def test_oauth_token(session: AsyncSession, test_user: User) -> DriveOAuthToken:
    ...
    from sparkth.core_plugins.googledrive.oauth import encrypt_token
...
def mock_drive_credentials() -> Generator[None, None, None]:
    ...
    with (
        patch("sparkth.core_plugins.googledrive.routes.oauth.get_drive_credentials", return_value=creds),
        patch("sparkth.core_plugins.googledrive.routes.folders.get_drive_credentials", return_value=creds),
        patch("sparkth.core_plugins.googledrive.routes.files.get_drive_credentials", return_value=creds),
    ):
        yield
...
def mock_valid_access_token() -> Generator[None, None, None]:
    ...
    with (
        patch("sparkth.core_plugins.googledrive.routes.oauth.get_valid_access_token", return_value="fake_access_token"),
        patch(
            "sparkth.core_plugins.googledrive.routes.folders.get_valid_access_token", return_value="fake_access_token"
        ),
        patch("sparkth.core_plugins.googledrive.routes.files.get_valid_access_token", return_value="fake_access_token"),
    ):
        yield
