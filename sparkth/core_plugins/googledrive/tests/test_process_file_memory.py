...
from sparkth.core_plugins.googledrive.utils import _process_single_file, _process_with_semaphore
...
class TestProcessFileMemory:
    ...
    async def test_malloc_trim_not_called_on_darwin(self) -> None:
        ...
        with (
            ...
            patch("sparkth.core_plugins.googledrive.utils._download_file", new=AsyncMock(return_value=b"%PDF-1.4\n")),
            ...
            patch(
                "sparkth.core_plugins.googledrive.utils.create_document", new=AsyncMock(return_value=MagicMock(id=1))
            ),
            patch(
