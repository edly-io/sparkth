...
from sparkth.core_plugins.chat.routes.utils import parse_metadata_list
from sparkth.core_plugins.chat.routes.utils.stream_processor import ChatStreamProcessor
from sparkth.core_plugins.chat.schemas import ChatCompletionRequest, ChatMessage, MessageResponse
...
async def test_status_events_emitted_before_tokens() -> None:
    ...
    with (
        patch(
            "sparkth.core_plugins.chat.routes.utils.stream_processor.agentic_retrieve_context", new_callable=AsyncMock
        ) as mock_retrieve,
    ):
