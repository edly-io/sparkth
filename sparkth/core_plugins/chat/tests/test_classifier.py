...
from sparkth.core_plugins.chat.classifier import HistoryTurn, ScopeClassifier, _ScopeResult
...
def _build_classifier(provider: str = "anthropic") -> tuple["ScopeClassifier", AsyncMock]:
    ...
    """
    chat_cls = {
        "anthropic": "sparkth.core_plugins.chat.classifier.ChatAnthropic",
        "openai": "sparkth.core_plugins.chat.classifier.ChatOpenAI",
        "google": "sparkth.core_plugins.chat.classifier.ChatGoogleGenerativeAI",
    }[provider]
...
class TestScopeClassifierInit:
    ...
    def test_anthropic_uses_haiku_model(self) -> None:
        with patch("sparkth.core_plugins.chat.classifier.ChatAnthropic") as MockChat:
            mock_llm = MagicMock()
    ...
    def test_openai_uses_mini_model(self) -> None:
        with patch("sparkth.core_plugins.chat.classifier.ChatOpenAI") as MockChat:
            mock_llm = MagicMock()
            mock_llm.with_structured_output.return_value = MagicMock()
            MockChat.return_value = mock_llm
    ...
    def test_google_uses_flash_model(self) -> None:
        with patch("sparkth.core_plugins.chat.classifier.ChatGoogleGenerativeAI") as MockChat:
            mock_llm = MagicMock()
            mock_llm.with_structured_output.return_value = MagicMock()
            MockChat.return_value = mock_llm
