...
from sparkth.core_plugins.slack.config import SlackConfig
...
from sparkth.core_plugins.slack.exceptions import UserAlreadyConnectedError, WorkspaceAlreadyConnectedError
from sparkth.core_plugins.slack.models import (
    BotResponseLog,
    SlackConnectionLog,
    SlackWorkspace,
)
from sparkth.core_plugins.slack.oauth import generate_state
from sparkth.core_plugins.slack.routes import _dispatch_event
from sparkth.core_plugins.slack.service import WorkspaceService
...
class TestOAuthCallback:
    ...
    async def test_valid_callback_saves_workspace_and_redirects(
        ...
        with patch(
            "sparkth.core_plugins.slack.routes.oauth.exchange_code_for_tokens",
    ...
    async def test_expired_state_returns_400(self, slack_client: AsyncClient) -> None:
        with patch(
            "sparkth.core_plugins.slack.routes.oauth.decode_state",
...
class TestSlackEvents:
    ...
    async def test_url_verification_returns_challenge(self, slack_client: AsyncClient) -> None:
        ...
        with patch("sparkth.core_plugins.slack.routes.get_slack_settings") as mock_settings:
    ...
    async def test_event_callback_dispatches_background_task(
        ...
        with (
            patch("sparkth.core_plugins.slack.routes.get_slack_settings") as mock_settings,
            patch("sparkth.core_plugins.slack.routes._dispatch_event", new_callable=AsyncMock) as mock_dispatch,
        ):
            mock_settings.return_value.signing_secret = ""
            response = await slack_client.post(
                "/api/v1/slack/events",
...
class TestDispatchEvent:
    ...
    async def test_posts_not_configured_message_when_plugin_missing(self) -> None:
        ...
        with (
            patch("sparkth.core_plugins.slack.routes.PluginService", return_value=mock_plugin_svc),
            patch("sparkth.core_plugins.slack.routes.decrypt_token", return_value="xoxb-fake"),
            patch("sparkth.core_plugins.slack.routes.SlackClient", return_value=slack_client),
        ):
    ...
    async def test_question_posts_rag_answer(self) -> None:
        ...
        with (
            ...
            patch("sparkth.core_plugins.slack.routes.SlackClient", return_value=slack_client),
            patch(
                "sparkth.core_plugins.slack.routes._build_llm_provider",
                new_callable=AsyncMock,
                return_value=mock_llm_provider,
            ),
            patch(
                "sparkth.core_plugins.slack.routes.answer_question",
                new_callable=AsyncMock,
    ...
    async def test_question_returns_ai_key_unavailable_when_build_llm_provider_fails(self) -> None:
        ...
        with (
            ...
            patch("sparkth.core_plugins.slack.routes.decrypt_token", return_value="xoxb-fake"),
            patch("sparkth.core_plugins.slack.routes.SlackClient", return_value=slack_client),
            patch("sparkth.core_plugins.slack.routes._build_llm_provider", new_callable=AsyncMock, return_value=None),
            patch(
                "sparkth.core_plugins.slack.routes.answer_question",
                new_callable=AsyncMock,
            ) as mock_answer,
...
class TestRagSources:
    ...
    async def test_returns_empty_list_when_user_has_no_sources(
        ...
        with patch(
            "sparkth.core_plugins.slack.routes.list_ready_documents",
            new_callable=AsyncMock,
            return_value=[],
        ):
...
async def test_dispatch_event_passes_llm_provider_when_configured() -> None:
    ...
    with (
        patch("sparkth.core_plugins.slack.routes.PluginService") as mock_plugin_svc,
        patch("sparkth.core_plugins.slack.routes.answer_question", new_callable=AsyncMock) as mock_aq,
        patch("sparkth.core_plugins.slack.routes.decrypt_token", return_value="xoxb-fake"),
        patch("sparkth.core_plugins.slack.routes.SlackClient", return_value=mock_slack_client),
        patch("sparkth.core_plugins.slack.routes.get_llm_service") as mock_get_llm_svc,
        patch("sparkth.core_plugins.slack.routes.get_provider") as mock_get_provider,
        patch("sparkth.core_plugins.slack.routes.get_slack_settings"),
    ):
...
class TestDispatchEventLogging:
    ...
    async def test_plugin_not_configured_logs_plugin_disabled(self, session: AsyncSession) -> None:
        with (
            patch(
                "sparkth.core_plugins.slack.routes.PluginService",
                return_value=MagicMock(get_user_plugin_map=AsyncMock(return_value={})),
            ),
            patch(
                "sparkth.core_plugins.slack.routes._post_slack_message",
                new_callable=AsyncMock,
                return_value="111.0",
            ),
            patch(
                "sparkth.core_plugins.slack.routes._resolve_names",
                new_callable=AsyncMock,
    ...
    async def test_greeting_logs_with_resolved_names(self, session: AsyncSession) -> None:
        ...
        with (
            ...
            patch("sparkth.core_plugins.slack.routes.is_greeting", return_value=True),
        ):
            await _dispatch_event(
                workspace_id=1,
                user_id=1,
