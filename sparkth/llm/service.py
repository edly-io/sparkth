...
class LLMConfigService:
    ...
    async def resolve(self, session: AsyncSession, user_id: int, config_id: int) -> tuple[LLMConfig, str]:
        ...
        audit event in the caller's transaction: the credential access trail. It
        rides the caller's session rather than committing on its own so the read
        costs no second pooled connection; both callers (chat and Slack) commit
        that session right after resolving, on their error paths too, so the
        record lands whenever the turn is persisted at all.
        """
        config = await self.get(session, user_id, config_id)
        if config is None:
            raise LLMConfigNotFoundError(config_id, user_id)
