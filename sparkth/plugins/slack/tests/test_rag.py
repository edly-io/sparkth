...
from sparkth.plugins.slack.enums import ResponseType
from sparkth.plugins.slack.rag import (
...
async def test_answer_question_returns_no_files_response_when_no_files_resolved() -> None:
    ...
    with patch("sparkth.plugins.slack.rag._resolve_document_ids_for_sources", new_callable=AsyncMock) as resolver:
...
async def test_answer_question_returns_file_not_found_on_drive_file_not_found_error() -> None:
    ...
    with (
        patch("sparkth.plugins.slack.rag._resolve_document_ids_for_sources", new_callable=AsyncMock) as resolver,
        patch("sparkth.plugins.slack.rag.agentic_retrieve_context", new_callable=AsyncMock) as mock_retrieve,
    ):
...
async def test_answer_question_synthesizes_when_llm_provider_set() -> None:
    ...
    with (
        patch("sparkth.plugins.slack.rag._resolve_document_ids_for_sources", new_callable=AsyncMock) as resolver,
        patch("sparkth.plugins.slack.rag.agentic_retrieve_context", new_callable=AsyncMock) as mock_retrieve,
        patch("sparkth.plugins.slack.rag.synthesize_answer", new_callable=AsyncMock) as synth,
    ):
        resolver.return_value = [10]
