...
```python
# sparkth/core_plugins/myappplugin/config.py
...
```python
# sparkth/core_plugins/myappplugin/plugin.py

from sparkth.core_plugins.myappplugin.config import MyAppPluginConfig
...
```python
# sparkth/core_plugins/myappplugin/adapter.py
...
```python
# sparkth/core_plugins/myappplugin/plugin.py
from sparkth.core_plugins.myappplugin.adapter import MyAppPluginConfigAdapter
from sparkth.core_plugins.myappplugin.config import MyAppPluginConfig
...
You do **not** choose the plugin name freely. The `PluginLoader` instantiates each plugin with a name it derives from the class name (`_class_name_to_plugin_name` in `sparkth/plugins/loader.py`): it strips a trailing `Plugin` suffix and kebab-cases the rest.
...
The loader constructs every plugin as `plugin_class(plugin_name)` (`sparkth/plugins/loader.py`), so `__init__` **must accept the derived `plugin_name` as its first positional argument** and pass it straight through to `super().__init__()`. Do not hard-code the name.
...
from sparkth.core_plugins.my_app.models import MyModel  # noqa: F401
