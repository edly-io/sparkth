...
# than at import time so that (a) importing `sparkth.core.db` has no side effects and
...
def get_engine() -> AsyncEngine:
    ...
    All database access goes through this provider (via `sparkth.lib.db.session_scope`
...
async def open_session(expire_on_commit: bool = False) -> AsyncGenerator[AsyncSession, None]:
    ...
    This is the single low-level session provider that :func:`sparkth.lib.db.session_scope`
