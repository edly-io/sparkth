"""Org-tree table models. Authored with LLM (Claude) assistance."""
...
class OrgUnit(TimestampedModel, table=True):
    """A node in the organisation tree (university, faculty, department, …).
    ...
    parent_id: int | None = Field(default=None, foreign_key="org_unit.id", index=True)
    path: str = Field(max_length=500, index=True)


class OrgMembership(TimestampedModel, SoftDeleteModel, table=True):
    """A user's seat in an org unit — HR truth, not a permission lever.
    ...
    org_unit_id: int = Field(foreign_key="org_unit.id", index=True)
