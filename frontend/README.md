...
- `name` – unique plugin identifier (must match folder and route, **and** the backend plugin's derived name — see "Plugin Name Derivation" in [`sparkth/plugins/PLUGIN_GUIDE.md`](../sparkth/plugins/PLUGIN_GUIDE.md) — so the UI resolves to the right backend plugin)
