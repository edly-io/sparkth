"""Tests for org membership management (sparkth.core.organization.memberships).
...
from sparkth.core.organization import memberships, units
from sparkth.core.organization.exceptions import OrganizationalUnitNotFound
...
async def test_add_member_is_idempotent(session: AsyncSession) -> None:
    ...
    unit = await units.create_organizational_unit("CS Dept", None, None, session)
    assert user.id is not None and unit.id is not None
    first = await memberships.add_organization_member(user.id, unit.id, session)
    second = await memberships.add_organization_member(user.id, unit.id, session)
...
async def test_add_member_missing_unit_raises(session: AsyncSession) -> None:
    ...
    with pytest.raises(OrganizationalUnitNotFound):
        await memberships.add_organization_member(user.id, 999, session)
...
async def test_remove_member_soft_deletes(session: AsyncSession) -> None:
    ...
    unit = await units.create_organizational_unit("CS Dept", None, None, session)
    assert user.id is not None and unit.id is not None
    await memberships.add_organization_member(user.id, unit.id, session)
    await memberships.remove_organization_member(user.id, unit.id, session)
    assert await memberships.get_organization_members(unit.id, session) == []
    rows = (
        await session.exec(
...
async def test_get_members_lists_active(session: AsyncSession) -> None:
    ...
    await memberships.add_organization_member(alice.id, unit.id, session)
    await memberships.add_organization_member(bob.id, unit.id, session)
    assert sorted(await memberships.get_organization_members(unit.id, session)) == sorted([alice.id, bob.id])
...
async def test_membership_is_not_inherited_by_descendants(session: AsyncSession) -> None:
    ...
    user = await make_user(session, "alice")
    parent = await units.create_organizational_unit("Faculty", None, None, session)
    assert parent.id is not None
    child = await units.create_organizational_unit("CS Dept", None, parent.id, session)
    assert user.id is not None and child.id is not None
    await memberships.add_organization_member(user.id, parent.id, session)
    assert await memberships.get_organization_members(child.id, session) == []
