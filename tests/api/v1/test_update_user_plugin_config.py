...
def test_update_plugin_not_configured_success(override_dependencies: Any) -> None:
...
def test_update_plugin_not_found(override_dependencies: Any) -> None:
    ...
    response = client.put("/api/v1/user-plugins/plugin_not_found/config", json=payload)

    assert response.status_code == status.HTTP_404_NOT_FOUND


def test_update_admin_disabled_plugin(override_dependencies: Any) -> None:
    client = override_dependencies
    payload = {"config": {"some_config": 123}}

    response = client.put("/api/v1/user-plugins/disabled_plugin/config", json=payload)

    assert response.status_code == status.HTTP_403_FORBIDDEN


def test_update_user_disabled_plugin(override_dependencies: Any) -> None:
    client = override_dependencies
    payload = {"config": {"some_config": 123}}

    response = client.put("/api/v1/user-plugins/configured_plugin_disabled/config", json=payload)

    assert response.status_code == status.HTTP_409_CONFLICT


def test_update_user_plugin_invalid_config(override_dependencies: Any) -> None:
    client = override_dependencies
