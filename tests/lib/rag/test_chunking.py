from app.lib.rag.chunking import DocumentChunker, chunk_document
from app.lib.rag.enums import DocType
from app.lib.rag.extraction import ExtractionResult
...
class TestBuildMetadata:
    def test_all_fields_mapped(self) -> None:
        ...
        m = DocumentChunker._build_metadata(raw, "doc.pdf")
    ...
    def test_missing_fields_default_to_none(self) -> None:
        m = DocumentChunker._build_metadata({}, "doc.pdf")
        assert m.chapter is None
        assert m.section is None
        assert m.subsection is None

    def test_partial_fields(self) -> None:
        m = DocumentChunker._build_metadata({"chapter": "Ch1"}, "doc.pdf")
        assert m.chapter == "Ch1"
        assert m.section is None
...
class TestSplit:
    ...
    def test_returns_list_of_chunks(self) -> None:
        chunks = self.chunker._split(STRUCTURED_MD, "test.md")
    ...
    def test_flat_document_returns_single_chunk(self) -> None:
        chunks = self.chunker._split(FLAT_MD, "flat.md")
        assert len(chunks) == 1
...
class TestCountTokens:
    def test_returns_int(self) -> None:
        assert isinstance(DocumentChunker._count_tokens("hello"), int)

    def test_empty_string_is_zero(self) -> None:
        assert DocumentChunker._count_tokens("") == 0

    def test_scales_with_length(self) -> None:
        short = DocumentChunker._count_tokens("hello")
        long = DocumentChunker._count_tokens("hello " * 100)
        assert long > short
...
class TestSecondaryChunking:
    ...
    def test_oversized_chunk_is_split_into_multiple(self) -> None:
        sub = self.chunker._secondary_split(_big_chunk())
        assert len(sub) > 1
    ...
    def test_small_chunk_returns_single_item(self) -> None:
        sub = self.chunker._secondary_split(_small_chunk())
        assert len(sub) == 1
    ...
    def test_split_applies_secondary_for_large_document(self) -> None:
        ...
        md = f"# Big Section\n\n{_LARGE_TEXT}\n"
        chunks = self.chunker._split(md, "big.md")
        assert len(chunks) > 1

    def test_split_does_not_split_small_sections(self) -> None:
        chunks = self.chunker._split(STRUCTURED_MD, "test.md")
        assert all(DocumentChunker._count_tokens(c.content) <= 512 for c in chunks)
