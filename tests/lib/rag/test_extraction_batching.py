...
class TestBatchingLoop:
    def test_single_batch_when_pages_fit(self) -> None:
        ...
        with (
            patch("app.lib.rag.extraction.pdf.fitz.open", return_value=_make_doc(3)),
            patch("app.lib.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.lib.rag.extraction.pdf.pymupdf4llm.to_markdown", return_value="md") as mock_to_md,
        ):
    ...
    def test_evenly_divisible_doc(self) -> None:
        ...
        with (
            patch("app.lib.rag.extraction.pdf.fitz.open", return_value=_make_doc(20)),
            patch("app.lib.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.lib.rag.extraction.pdf.pymupdf4llm.to_markdown", return_value="md") as mock_to_md,
        ):
    ...
    def test_last_partial_batch(self) -> None:
        ...
        with (
            patch("app.lib.rag.extraction.pdf.fitz.open", return_value=_make_doc(25)),
            patch("app.lib.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.lib.rag.extraction.pdf.pymupdf4llm.to_markdown", return_value="md") as mock_to_md,
        ):
            _extract_pdf(b"%PDF-fake", "doc.pdf", batch_size=10)
    ...
    def test_batch_size_kwarg_overrides_setting(self) -> None:
        ...
        with (
            patch("app.lib.rag.extraction.pdf.fitz.open", return_value=_make_doc(6)),
            patch("app.lib.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.lib.rag.extraction.pdf.pymupdf4llm.to_markdown", return_value="md") as mock_to_md,
        ):
    ...
    def test_batches_concatenated_in_order(self) -> None:
        ...
        with (
            ...
            patch("app.lib.rag.extraction.pdf.fitz.TOOLS"),
            patch(
                "app.lib.rag.extraction.pdf.pymupdf4llm.to_markdown",
