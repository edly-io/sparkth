...
from app.memory_profiler import log_memory_snapshot, profile_memory
...
def mock_log_file_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("app.memory_profiler.LOG_FILE_PATH", temp_log)
    monkeypatch.setattr("app.memory_profiler.LOG_DIR", tmp_path)
    monkeypatch.setattr("app.memory_profiler._memprof_handler", None)
    monkeypatch.setattr("app.memory_profiler._memprof_logger", None)
    return temp_log
