...
from app.lib.rag.agent import RAGSearchAgent
from app.lib.rag.exceptions import RAGRetrievalError
from app.lib.rag.schemas import RAGSearchAgentResponse, SectionRef
...
class TestRAGSearchAgent:
    ...
    def _patch_agent(self, agent: Any) -> Any:
        return patch("app.lib.rag.agent.create_agent", return_value=agent)
    ...
    def _patch_mcp_client(self, side_effect: Exception | None = None) -> Any:
        ...
        return patch("app.lib.rag.agent.MultiServerMCPClient", mock_cls)

    def _patch_settings(self) -> Any:
        mock_settings = MagicMock()
        mock_settings.RAG_MCP_URL = "http://test-mcp:7728/mcp"
        return patch("app.lib.rag.agent.get_settings", return_value=mock_settings)
