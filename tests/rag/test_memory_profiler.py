...
def mock_log_file_path(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> Path:
    ...
    monkeypatch.setattr("sparkth.memory_profiler.LOG_FILE_PATH", temp_log)
    monkeypatch.setattr("sparkth.memory_profiler.LOG_DIR", tmp_path)
    monkeypatch.setattr("sparkth.memory_profiler._memprof_handler", None)
    monkeypatch.setattr("sparkth.memory_profiler._memprof_logger", None)
    return temp_log
...
class TestProfileMemoryDisabled:
    ...
    async def test_no_log_when_disabled(
        ...
        monkeypatch.setattr(
            "sparkth.memory_profiler.get_settings",
