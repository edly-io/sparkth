...
class TestExtractPDFRejectsScanned:
    def test_raises_scanned_pdf_error_when_doc_has_no_text(self) -> None:
        ...
        with (
            patch("app.rag.extraction.pdf.fitz.open", return_value=scanned_doc),
            patch("app.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.rag.extraction.pdf.pymupdf4llm.to_markdown") as mock_to_md,
        ):
    ...
    def test_error_attaches_source_name_to_exception(self) -> None:
        ...
        with (
            patch("app.rag.extraction.pdf.fitz.open", return_value=scanned_doc),
            patch("app.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.rag.extraction.pdf.pymupdf4llm.to_markdown"),
        ):
    ...
    def test_text_pdf_not_rejected(self) -> None:
        ...
        with (
            patch("app.rag.extraction.pdf.fitz.open", return_value=text_doc),
            patch("app.rag.extraction.pdf.fitz.TOOLS"),
            patch("app.rag.extraction.pdf.pymupdf4llm.to_markdown", return_value="content") as mock_to_md,
        ):
    ...
    def test_threshold_is_read_from_settings(self) -> None:
        ...
        with (
            patch("app.rag.extraction.pdf.get_rag_settings") as mock_settings,
            patch("app.rag.extraction.pdf.fitz.open", return_value=borderline_doc),
            patch("app.rag.extraction.pdf.fitz.TOOLS"),
