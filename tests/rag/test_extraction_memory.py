...
class TestExtractionMemory:
    def test_store_shrunk_after_single_batch(self) -> None:
        ...
        with (
            patch("sparkth.rag.ingestion.extraction.pdf.fitz.open", return_value=_make_mock_doc(3)),
            patch("sparkth.rag.ingestion.extraction.pdf.fitz.TOOLS") as mock_tools,
    ...
    def test_store_shrunk_once_per_batch(self) -> None:
        ...
        with (
            patch("sparkth.rag.ingestion.extraction.pdf.fitz.open", return_value=_make_mock_doc(25)),
            patch("sparkth.rag.ingestion.extraction.pdf.fitz.TOOLS") as mock_tools,
            patch("sparkth.rag.ingestion.extraction.pdf.pymupdf4llm.to_markdown", return_value="md"),
        ):
    ...
    def test_store_shrunk_even_on_extraction_error(self) -> None:
        ...
        with (
            patch("sparkth.rag.ingestion.extraction.pdf.fitz.open", return_value=_make_mock_doc(5)),
            patch("sparkth.rag.ingestion.extraction.pdf.fitz.TOOLS") as mock_tools,
            patch(
