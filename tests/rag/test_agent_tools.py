...
class TestBuildSearchTools:
    ...
    def test_builds_all_six_tools_with_expected_names(self) -> None:
        tools = build_search_tools(user_id=1, file_id=2)
    ...
    def test_all_tools_have_non_empty_descriptions(self) -> None:
        for tool in build_search_tools(user_id=1, file_id=2):
            assert isinstance(tool.description, str)
            assert tool.description.strip()

    def test_context_only_tools_expose_no_llm_args(self) -> None:
        tools = _by_name(build_search_tools(user_id=1, file_id=2))
        for name in (
            "list_user_files",
            "get_file_metadata",
            "list_file_sections",
    ...
    def test_search_tool_exposes_only_keyword(self) -> None:
        tool = _by_name(build_search_tools(user_id=1, file_id=2))["search_section_by_keyword"]
